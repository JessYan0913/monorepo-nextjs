...
import { staffDetail, updateStaff } from "@/lib/actions/staff"
...
export default function StaffEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: staff = mockStaffData, isLoading } = useSWR(
    isAddMode ? null : `staff-${id}`,
    async () => {
      const data = await staffDetail(id)
      if (!data) {
        notFound()
      }
      return data
  ...
  const form = useForm<StaffFormValues>({
    ...
    defaultValues: {
      staffId: isAddMode ? 0 : staff.staffId,
      ...
      <Card className="overflow-hidden border border-border/50 shadow-sm transition-all hover:shadow-md">
        ...
        <Form {...form}>
          <form onSubmit={submit}>
            <input type="hidden" {...form.register("staffId")} value={id} />
