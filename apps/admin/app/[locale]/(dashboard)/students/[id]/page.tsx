...
import { studentDetail, updateStudent } from "@/lib/actions/student"
...
const mockStudentData = {
  ...
  studentGrade: "2022级",
  studentProfilePicture: "https://gips2.baidu.com/it/u=1840289963,2728468038&fm=3042&app=3042&f=JPEG&wm=1,baiduai,0,0,13,9&wmo=0,0&w=480&h=640",
  createId: 1,
  createName: "admin",
...
export default function StudentEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: student = mockStudentData, isLoading } = useSWR(
    isAddMode ? null : `student-${id}`,
    async () => {
      const data = await studentDetail(id)
      if (!data) {
        notFound()
      }
      return data
  ...
  const form = useForm<StudentFormValues>({
    ...
    defaultValues: {
      studentId: isAddMode ? 0 : student.studentId,
